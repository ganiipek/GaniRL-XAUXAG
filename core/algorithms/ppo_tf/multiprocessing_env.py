...
class Environment(Process):
    def __init__(self, env_idx, child_conn, env, training_batch_size):
...
def train_multiprocessing(CustomEnv, agent, train_df, num_worker=4, training_batch_size=512, EPISODES=10000):
    ...
    for idx in range(num_worker):
        ...
        env = CustomEnv(data=train_df, verbose=True)
        work = Environment(idx, child_conn, env, training_batch_size)
        work.start()
        works.append(work)
    ...
    while episode < EPISODES:
        print(np.reshape(state, [num_worker]+[_ for _ in state[0].shape]))
        predictions_list = agent.choose_action(np.reshape(state, [num_worker]+[_ for _ in state[0].shape]))
        # predictions_list = agent.choose_action(np.reshape(state, [num_worker]+[_ for _ in state[0].shape]))
        # actions_list = [np.random.choice(agent.action_space, p=i) for i in predictions_list]
        print(predictions_list)
        actions_list = [np.random.choice([0, 1, 2], p=i[0]) for i in predictions_list]
        print(actions_list)
